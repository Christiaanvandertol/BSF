...
for iprior = 1:1
    ...
    for k = 1:length(dirs)
        for m = 1:2
            fileinfo = dir([datapath '/' dirs{k} '/crane_flox/' m_name{m} '.csv']);
           % fileinfo2 = dir([datapath '/' dirs{k} '/crane_flox/' m_name{m} '_radiance_FULL_*.csv']);
            ...
            if ifdata
                D(m).filename = [datapath '/' dirs{k} '/crane_flox/' fileinfo(1).name];%#ok<*SAGROW> %[datapath '/' dirs(k).name '/' m_name{m} '_radiance_FLUO_*.csv']; %#ok<*SAGROW>
